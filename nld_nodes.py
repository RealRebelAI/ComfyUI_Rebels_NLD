...
class GGUFLinearWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if out_features >= GGUFLinearWrapper.BIG_OUT:
            # keep the quantized head on its home device; transfer per row-chunk below.
            # Shipping the whole tensor first costs a ~570MB VRAM staging alloc (Q8 head)
            # on every call and forfeits copy/compute overlap.
            q = self.qdata
            # Chunk BOTH the dequant and the matmul over output rows. Dequantizing the
            # whole 131072x4096 weight first would allocate ~1GB in bf16 - the reason
            # Q6/Q8 heads OOM'd. Each row is a whole number of quant blocks (in_features
            # 4096 = 16 K-superblocks = 128 Q8_0 blocks), so row slicing is block-safe.
            in_features = int(self.oshape[1])
            bytes_per_row = q.numel() // out_features
            qrows = q.view(out_features, bytes_per_row)
            b = self.bias.to(x.device, x.dtype) if self.bias is not None else None
            ...
            for i in range(0, out_features, step):
                ...
                wq = qrows[i:j].reshape(-1)
                if wq.device != x.device:
                    wq = wq.to(x.device, non_blocking=True)
                else:
                    wq = wq.contiguous()
                w = dequant_bytes(wq, self.qtype, (j - i, in_features), x.dtype)
                bi = b[i:j] if b is not None else None
                out[..., i:j] = F.linear(x, w, bi)
                del w, wq
            del qrows
            return out
...
class NLDLoaderGGUF:
    _pinned_bytes = 0
    PIN_BUDGET = 6 * (1024 ** 3)  # cap pinned (non-pageable) RAM at 6GB on a 16GB box
    ...
    def load_model(self, gguf_name, vqvae_name, device, weights_location, attention="auto"):
        ...
        for gname, (qbytes, qtype, oshape) in gg.items():
            ...
            if is_quant and isinstance(parent_module, torch.nn.Embedding) and param_name == "weight":
                ...
                qb = qbytes.to(qdev)
                if qdev == "cpu" and NLDLoaderGGUF._pinned_bytes < NLDLoaderGGUF.PIN_BUDGET:
                    # PINNED memory makes the per-forward H2D copy truly async.
                    # From pageable memory, non_blocking=True is silently synchronous,
                    # which serializes ~5.5GB of transfers per forward pass. Best-effort
                    # and budget-capped: pinned RAM is non-pageable, and pinning a 9.6GB
                    # Q8 file on a 16GB Windows box would starve the OS.
                    try:
                        qb = qb.pin_memory()
                        NLDLoaderGGUF._pinned_bytes += qb.numel()
                    except RuntimeError:
                        pass
                wrapper = GGUFLinearWrapper(qb, qtype, oshape, bias=bias)
                gp_name, child = (parent_name.rsplit(".", 1) + [""])[:2] if "." in parent_name else ("", parent_name)
                gp = modules_dict.get(gp_name, model) if gp_name else model
                setattr(gp, child if gp_name else parent_name, wrapper)
                attached += 1
        ...
        if NLDLoaderGGUF._pinned_bytes:
            print("[NLD] pinned {:.2f} GB of streaming weights (async H2D transfers)".format(
                NLDLoaderGGUF._pinned_bytes / 1e9))
            NLDLoaderGGUF._pinned_bytes = 0
        if unmatched:
...
class NLDTextToImage:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "sample_policy": (["argmax", "multinomial"], {"default": "argmax"}),
                "alg_temp": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 2.0, "step": 0.05}),
                "top_k": ("INT", {"default": 0, "min": 0, "max": 4096}),
                "top_p": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01}),
                "shift": ("INT", {"default": 3, "min": 1, "max": 8}),
            }
        }
    ...
    def generate(self, nld_model, prompt, width, height, steps, guidance, temperature, seed,
                 decode_device="gpu", dequant_cache_gb=0.0, kv_cache="off (flat step time)",
                 sample_policy="argmax", alg_temp=0.0, top_k=0, top_p=1.0, shift=3):
        ...
        print("[NLD] sampling: policy={} alg_temp={} top_k={} top_p={} shift={}".format(
            sample_policy, alg_temp, top_k or "off", top_p if top_p < 1.0 else "off", shift))
        ...
        try:
            with torch.inference_mode():
                out = model.text_to_image(
                    ...
                    # --- token choice / commit order ---------------------------------
                    # NVIDIA's defaults (multinomial + alg_temp=1.0) randomize BOTH which
                    # token is chosen and the order tokens are committed. That is fine for
                    # sample diversity and terrible for glyphs and fine structure:
                    #   * multinomial: an 80%-confident codebook token is replaced by a
                    #     different 16x16 patch 20% of the time.
                    #   * alg_temp=1.0: confidence = log(p) + gumbel(std~1.28), while the
                    #     log-p spread across tokens is only ~4 -> commit order is largely
                    #     noise. Wrong glyphs lock in early and everything after adapts.
                    # argmax + alg_temp=0.0 = take the best token, commit the most certain
                    # ones first, let ambiguous regions resolve once neighbours exist.
